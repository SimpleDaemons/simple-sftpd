...
useradd -m -s /bin/bash sftpddev || true
usermod -aG sudo sftpddev || true

# Create project directory
echo "Setting up project directory..."
PROJECT_DIR="/opt/simple-sftpd"
mkdir -p $PROJECT_DIR
chown sftpddev:sftpddev $PROJECT_DIR

# Create log directories
echo "Creating log directories..."
mkdir -p /var/log/simple-sftpd
chown sftpddev:sftpddev /var/log/simple-sftpd

# Create config directory
echo "Creating config directory..."
mkdir -p /etc/simple-sftpd
chown sftpddev:sftpddev /etc/simple-sftpd
...
echo "2. Run: cd $PROJECT_DIR && sudo -u sftpddev ./build.sh"
echo "3. Test with: sudo -u sftpddev make test"
