#!/bin/bash
# Generic Linux build script for simple-sftpd
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-sftpd.service > /dev/null << EOF
[Unit]
Description=Simple Secure FTP Daemon - A secure, configurable, and feature-rich FTP server
...
User=simple-sftpd
Group=simple-sftpd
ExecStart=/usr/local/bin/simple-sftpd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-sftpd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-sftpd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-sftpd - Linux Build Script
