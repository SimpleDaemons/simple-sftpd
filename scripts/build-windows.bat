...
:build_project
call :print_status "Building simple-sftpd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-sftpd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-sftpd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-sftpd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-sftpd-0.1.0-static-windows' -DestinationPath 'simple-sftpd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-sftpd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-sftpd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-sftpd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-sftpd build process..."
...
call :print_success "simple-sftpd build process completed successfully!"
goto :end
