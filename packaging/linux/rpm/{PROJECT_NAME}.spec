...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-sftpd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-sftpd is a modern, secure implementation of the FTP protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id sftpddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" sftpddev
    fi
fi
