#!/bin/bash
# Create DMG installer for simple-sftpd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-sftpd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-sftpd.app"
PKG_NAME="$simple-sftpd.pkg"
...
if [ -f "../assets/icons/$simple-sftpd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-sftpd.icns"
fi
